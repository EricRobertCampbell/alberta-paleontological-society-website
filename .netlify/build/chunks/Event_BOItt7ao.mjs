import { c as createComponent, b as createAstro, m as maybeRenderHead, a as renderTemplate, d as addAttribute, h as renderSlot, r as renderComponent } from './astro/server_S4n-JgDR.mjs';
import 'kleur/colors';
import { g as generateEventDateTimeString, e as extractReferenceId } from './functions_B7tnQCwS.mjs';
import { s as splitIsoString } from './dates_BDkwdIRj.mjs';
import { a as getEntry } from './_astro_content_CaQLvW2e.mjs';
import 'clsx';
/* empty css                            */
...
const $$Event = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const resolvedTalks = talks ? (await Promise.all(
    talks.map(async (talkRef) => {
      const actualTalkRef = extractReferenceId(talkRef);
      if (!actualTalkRef) {
        return null;
      }
      try {
        const talk = await getEntry("talks", actualTalkRef);
        return talk;
      } catch (e) {
        console.error("Error getting talk with ref", talkRef, e);
        return null;
      }
    })
  )).filter((t) => !!t) : [];
