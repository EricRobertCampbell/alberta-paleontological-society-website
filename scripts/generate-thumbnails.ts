 ...
 */
const THUMBNAIL_SIZE: { width: number; height: number } = { width: 220, height: 180 };
...
async function generateThumbnails(): Promise<void> {
	try {
		...
		const imageFiles = files.filter(
			(f: string) => /\.(jpg|jpeg|png)$/i.test(f) && !f.includes('-thumbnail.')
		);
		...
		for (const file of imageFiles) {
			...
			try {
				await sharp(inputPath)
					...
					.toFile(outputPath);
				console.log(`✓ Created thumbnail: ${file} → ${basename(outputPath)}`);
			} catch (error) {
